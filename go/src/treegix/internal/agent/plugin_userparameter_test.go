...
var results = []Result{
	Result{data: []string{"system.test,who | wc -l",
		...
		"proc.cpu[*],proc=\"$1\"; ps -o pcpu= -C \"${proc:-treegix_agentd}\" | awk '{sum += $$1} END {print sum}",
...
var resultsCmd = []Result{
	Result{data: []string{"system.test,who | wc -l",
		...
		input: []Input{
			...
			Input{key: "proc.cpu", params: []string{"foo"}, cmd: "proc=\"foo\"; ps -o pcpu= -C \"${proc:-treegix_agentd}\" | awk '{sum += $foo} END {print sum}"},
