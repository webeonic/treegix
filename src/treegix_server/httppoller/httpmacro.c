			...
			const char *pvalue, size_t nvalue, const char *data, char **err_str)
{
	...
	if (0 == nkey && 0 != nvalue)
	{
		treegix_log(LOG_LEVEL_DEBUG, "%s() missing variable name (only value provided): \"%.*s\"",
	...
	if (0 == strncmp(REGEXP_PREFIX, value_str, REGEXP_PREFIX_SIZE))
	{
		...
		{
			treegix_log(LOG_LEVEL_DEBUG, "%s() cannot extract the value of \"%.*s\" from response",
...
int	http_substitute_variables(const zbx_httptest_t *httptest, char **data)
{
	...
	treegix_log(LOG_LEVEL_DEBUG, "End of %s() data:'%s'", __func__, *data);
		...
		char **err_str)
{
	...
	treegix_log(LOG_LEVEL_DEBUG, "In %s() %d variables", __func__, variables->values_num);
