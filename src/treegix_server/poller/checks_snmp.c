 ...
 * -----------
 *
 * Treegix caches the whole index table for the particular OID separately based on:
 *   * IP address;
 *   * port;
 *   * community string (SNMPv2c);
 *   * context, security name (SNMPv3).
 *
 * Treegix revalidates each index before using it to get a value and rebuilds the index cache for the OID if the
 * index is invalid.
 *
 * Example
 * -------
 ...
 *
 * SNMP OID as configured in Treegix to get memory usage of "snmpd" process:
 *   HOST-RESOURCES-MIB::hrSWRunPerfMem["index","HOST-RESOURCES-MIB::hrSWRunPath","snmpd"]
 *
 * 1. Treegix walks hrSWRunPath table and caches all <PID> and <NAME> pairs of particular SNMP agent/user.
 * 2. Before each GET request Treegix revalidates the cached <PID> by getting its <NAME> from hrSWRunPath table.
 * 3. If the names match then Treegix uses the cached <PID> in the GET request for the hrSWRunPerfMem.
 *    Otherwise Treegix rebuilds the hrSWRunPath cache for the particular agent/user (see 1.).
 *
 * Implementation
 * --------------
 *
 ...
 *                                                                            *
 * Parameters: item      - [IN] configuration of Treegix item, contains        *
...
static struct snmp_session	*zbx_snmp_open_session(const DC_ITEM *item, char *error, size_t max_error_len)
{
	...
	if (SNMPERR_SUCCESS != netsnmp_ds_set_boolean(NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_DONT_CHECK_RANGE, 1))
	{
		...
		treegix_log(LOG_LEVEL_WARNING, "cannot set \"DontCheckRange\" option for Net-SNMP");
	}
	...
	if (SNMP_VERSION_1 == session.version || SNMP_VERSION_2c == session.version)
	{
		...
		treegix_log(LOG_LEVEL_DEBUG, "SNMP [%s@%s]", session.community, session.peername);
	}
	else if (SNMP_VERSION_3 == session.version)
	{
		...
		treegix_log(LOG_LEVEL_DEBUG, "SNMPv3 [%s@%s]", session.securityName, session.peername);
	}
...
static int	zbx_snmp_set_result(const struct variable_list *var, AGENT_RESULT *result)
{
	...
	treegix_log(LOG_LEVEL_DEBUG, "In %s() type:%d", __func__, (int)var->type);
...
static int	zbx_snmp_print_oid(char *buffer, size_t buffer_len, const oid *objid, size_t objid_len, int format)
{
	if (SNMPERR_SUCCESS != netsnmp_ds_set_boolean(NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_DONT_BREAKDOWN_OIDS,
			format))
	{
		treegix_log(LOG_LEVEL_WARNING, "cannot set \"dontBreakdownOids\" option to %d for Net-SNMP", format);
		return -1;
	}
		...
		size_t root_string_len, size_t root_numeric_len)
{
	...
	{
		treegix_log(LOG_LEVEL_DEBUG, "%s(): cannot parse OID '%s'", __func__, printed_oid);
		goto numeric;
	}
 ...
 *             item          - [IN] configuration of Treegix item              *
		...
		zbx_snmp_walk_cb_func walk_cb_func, void *walk_cb_arg)
{
	...
	while (1 == running)
	{
		...
		treegix_log(LOG_LEVEL_DEBUG, "%s() snmp_synch_response() status:%d s_snmp_errno:%d errstat:%ld"
		...
		for (num_vars = 0, var = response->variables; NULL != var; num_vars++, var = var->next_variable)
		{
			...
			else if (SNMP_NOSUCHOBJECT != var->type && SNMP_NOSUCHINSTANCE != var->type)
			{
				...
				else
				{
					...
					treegix_log(LOG_LEVEL_DEBUG, "cannot get index '%s' string value: %s",
		...
		char *error, size_t max_error_len, int *max_succeed, int *min_fail)
{
	...
	treegix_log(LOG_LEVEL_DEBUG, "In %s() num:%d level:%d", __func__, num, level);
...
retry:
	...
	treegix_log(LOG_LEVEL_DEBUG, "%s() snmp_synch_response() status:%d s_snmp_errno:%d errstat:%ld mapping_num:%d",
	...
	if (STAT_SUCCESS == status && SNMP_ERR_NOERROR == response->errstat)
	{
		for (i = 0, var = response->variables;; i++, var = var->next_variable)
		{
					...
					0 != memcmp(parsed_oids[j], var->name, parsed_oid_lens[j] * sizeof(oid)))
			{
				...
				{
					treegix_log(LOG_LEVEL_DEBUG, "SNMP response from host \"%s\" contains"
							" variable bindings that do not match the request:"
							" sent \"%s\", received \"%s\"",
							items[0].host.host, sent_oid, received_oid);
				}
	...
	else if (STAT_SUCCESS == status && SNMP_ERR_NOSUCHNAME == response->errstat && 0 != response->errindex)
	{
		...
		treegix_log(LOG_LEVEL_DEBUG, "%s() snmp_synch_response() errindex:%ld OID:'%s'", __func__,
...
void	get_values_snmp(const DC_ITEM *items, AGENT_RESULT *results, int *errcodes, int num)
{
	...
	treegix_log(LOG_LEVEL_DEBUG, "In %s() host:'%s' addr:'%s' num:%d",
...
exit:
	if (SUCCEED != err)
	{
		treegix_log(LOG_LEVEL_DEBUG, "getting SNMP values failed: %s", error);
