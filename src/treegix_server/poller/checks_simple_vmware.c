...
static trx_vmware_vm_t	*service_vm_get(trx_vmware_service_t *service, const char *uuid)
{
	...
	treegix_log(LOG_LEVEL_DEBUG, "End of %s():%p", __func__, (void *)vm);
...
static trx_vmware_cluster_t	*cluster_get(trx_vector_ptr_t *clusters, const char *clusterid)
{
	...
	cluster = NULL;
out:
	treegix_log(LOG_LEVEL_DEBUG, "End of %s():%p", __func__, (void *)cluster);
...
out:
	treegix_log(LOG_LEVEL_DEBUG, "End of %s():%p", __func__, (void *)service);
		...
		trx_vector_ptr_t *add_results)
{
	...
	treegix_log(LOG_LEVEL_DEBUG, "End of %s(): events:%d", __func__, add_results->values_num);
}
