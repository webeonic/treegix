...
static int	ssh_run(DC_ITEM *item, AGENT_RESULT *result, const char *encoding)
{
	...
	treegix_log(LOG_LEVEL_DEBUG, "%s() supported authentication methods:'%s'", __func__, userauthlist);
		...
		case ITEM_AUTHTYPE_PASSWORD:
			if (auth_pw & 1)
			{
				...
				if (0 != libssh2_userauth_password(session, item->username, item->password))
				{
					...
					treegix_log(LOG_LEVEL_DEBUG, "%s() password authentication succeeded", __func__);
			}
			else if (auth_pw & 2)
			{
				...
				if (0 != libssh2_userauth_keyboard_interactive(session, item->username, &kbd_callback))
				{
					...
					treegix_log(LOG_LEVEL_DEBUG, "%s() keyboard-interactive authentication succeeded",
							__func__);
			}
			else
			{
		...
		case ITEM_AUTHTYPE_PUBLICKEY:
			if (auth_pw & 4)
			{
				...
				if (0 != rc)
				{
					...
					treegix_log(LOG_LEVEL_DEBUG, "%s() authentication by public key succeeded",
							__func__);
			}
			else
			{
...
channel_close:
	...
	{
		libssh2_session_last_error(session, &ssherr, NULL, 0);
		treegix_log(LOG_LEVEL_WARNING, "%s() cannot close generic session channel: %s", __func__, ssherr);
	}
	else
		exitcode = libssh2_channel_get_exit_status(channel);

	treegix_log(LOG_LEVEL_DEBUG, "%s() exitcode:%d bytecount:%d", __func__, exitcode, bytecount);
