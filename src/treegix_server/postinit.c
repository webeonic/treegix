...
static int	update_event_names(void)
{
	...
	treegix_log(LOG_LEVEL_WARNING, "starting event name update forced by database upgrade");
	...
	while (SUCCEED == ret && NULL != (row = DBfetch(result)))
	{
		...
		{
			treegix_log(LOG_LEVEL_WARNING, "completed %d%% of event name update", completed);
...
out:
	if (SUCCEED == ret)
		...
		treegix_log(LOG_LEVEL_WARNING, "event name update failed");
