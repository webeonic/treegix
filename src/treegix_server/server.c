...
#ifdef HAVE_SQLITE3
#	error SQLite is not supported as a main Treegix database backend.
#endif
...
#include "events.h"
#include "../libs/trxdbcache/valuecache.h"
...
const char	title_message[] = "treegix_server";
const char	syslog_app_name[] = "treegix_server";
...
const char	*help_message[] = {
	"The core daemon of Treegix software.",
	"",
	"Options:",
	"  -c --config config-file        Path to the configuration file",
	"                                 (default: \"" DEFAULT_CONFIG_FILE "\")",
	"  -f --foreground                Run Treegix server in foreground",
...
char	*CONFIG_SERVER			= NULL;		/* not used in treegix_server, required for linking */
...
trx_uint64_t	CONFIG_TRENDS_CACHE_SIZE	= 4 * TRX_MEBIBYTE;
trx_uint64_t	CONFIG_VALUE_CACHE_SIZE		= 8 * TRX_MEBIBYTE;
trx_uint64_t	CONFIG_VMWARE_CACHE_SIZE	= 8 * TRX_MEBIBYTE;
trx_uint64_t	CONFIG_EXPORT_FILE_SIZE		= TRX_GIBIBYTE;
...
int	CONFIG_SERVER_STARTUP_TIME	= 0;	/* treegix server startup time */

int	CONFIG_PROXYPOLLER_FORKS	= 1;	/* parameters for passive proxies */

/* how often Treegix server sends configuration data to proxy, in seconds */
...
static void	trx_set_defaults(void)
{
	...
	if (NULL == CONFIG_PID_FILE)
		CONFIG_PID_FILE = trx_strdup(CONFIG_PID_FILE, "/tmp/treegix_server.pid");

	if (NULL == CONFIG_ALERT_SCRIPTS_PATH)
		CONFIG_ALERT_SCRIPTS_PATH = trx_strdup(CONFIG_ALERT_SCRIPTS_PATH, DEFAULT_ALERT_SCRIPTS_PATH);
	...
	if (NULL == CONFIG_HISTORY_STORAGE_OPTS)
		CONFIG_HISTORY_STORAGE_OPTS = trx_strdup(CONFIG_HISTORY_STORAGE_OPTS, "uint,dbl,str,log,text");
#endif
...
#	define TLS_FEATURE_STATUS	" NO"
	...
	treegix_log(LOG_LEVEL_INFORMATION, "SMTP authentication:       " SMTP_AUTH_FEATURE_STATUS);
	...
	if (SUCCEED != trx_vc_init(&error))
	{
		treegix_log(LOG_LEVEL_CRIT, "cannot initialize history value cache: %s", error);
		trx_free(error);
		exit(EXIT_FAILURE);
	}

	if (SUCCEED != trx_create_itservices_lock(&error))
	{
		treegix_log(LOG_LEVEL_CRIT, "cannot create IT services lock: %s", error);
		trx_free(error);
		exit(EXIT_FAILURE);
	}

	if (SUCCEED != trx_history_init(&error))
	{
		treegix_log(LOG_LEVEL_CRIT, "cannot initialize history storage: %s", error);
		trx_free(error);
		exit(EXIT_FAILURE);
	}

	if (FAIL == trx_export_init(&error))
	{
		treegix_log(LOG_LEVEL_CRIT, "cannot initialize export: %s", error);
		trx_free(error);
		exit(EXIT_FAILURE);
	}
	...
	if (SUCCEED != trx_check_postinit_tasks(&error))
	{
		treegix_log(LOG_LEVEL_CRIT, "cannot complete post initialization tasks: %s", error);
		trx_free(error);
		exit(EXIT_FAILURE);
	}

	/* update maintenance states */
	trx_dc_update_maintenances();
	...
	trx_tls_init_parent();
#endif
	treegix_log(LOG_LEVEL_INFORMATION, "server #0 started [main process]");
	...
	for (i = 0; i < threads_num; i++)
	{
		...
		switch (thread_args.process_type)
		{
			case TRX_PROCESS_TYPE_CONFSYNCER:
				trx_thread_start(dbconfig_thread, &thread_args, &threads[i]);
				break;
			...
			case TRX_PROCESS_TYPE_ESCALATOR:
				trx_thread_start(escalator_thread, &thread_args, &threads[i]);
				break;
			...
			case TRX_PROCESS_TYPE_PROXYPOLLER:
				trx_thread_start(proxypoller_thread, &thread_args, &threads[i]);
				break;
			...
			case TRX_PROCESS_TYPE_ALERTMANAGER:
				trx_thread_start(alert_manager_thread, &thread_args, &threads[i]);
				break;
			case TRX_PROCESS_TYPE_LLDMANAGER:
				trx_thread_start(lld_manager_thread, &thread_args, &threads[i]);
				break;
			case TRX_PROCESS_TYPE_LLDWORKER:
				trx_thread_start(lld_worker_thread, &thread_args, &threads[i]);
				break;
			case TRX_PROCESS_TYPE_ALERTSYNCER:
				trx_thread_start(alert_syncer_thread, &thread_args, &threads[i]);
				break;
		}
	}
	...
	{
		trx_history_export_init("main-process", 0);
		trx_problems_export_init("main-process", 0);
	}
