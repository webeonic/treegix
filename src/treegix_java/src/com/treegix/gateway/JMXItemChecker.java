...
class JMXItemChecker extends ItemChecker
{
	...
	JMXItemChecker(JSONObject request) throws TreegixException
	{
		super(request);
		...
		{
			throw new TreegixException("%s: %s", e, jmx_endpoint);
		}
	}
	...
	JSONArray getValues() throws TreegixException
	{
		...
		try
		{
			...
			jmxc = TreegixJMXConnectorFactory.connect(url, env);
		...
		catch (SecurityException e1)
		{
			...
			logger.warn("cannot process keys '{}': {}: {}", new Object[] {keys, TreegixException.getRootCauseMessage(e1), url});
		...
		{
			throw new TreegixException("%s: %s", TreegixException.getRootCauseMessage(e), url);
		}
		finally
		{
	...
	protected String getStringValue(String key) throws Exception
	{
		...
		{
			if (2 != argumentCount)
				throw new TreegixException("required key format: jmx[<object name>,<attribute name>]");
			...
			{
				throw new TreegixException("Object or attribute not found.");
			}
		}
		else if (item.getKeyId().equals("jmx.discovery") || item.getKeyId().equals("jmx.get"))
		{
			if (2 < argumentCount)
				throw new TreegixException("required key format: " + item.getKeyId() + "[<discovery mode>,<object name>]");
			...
			if (0 < argumentCount)
			{
				...
				if (modeName.equals("beans"))
					...
					throw new TreegixException("invalid discovery mode: " + modeName);
			}
	...
	private String getPrimitiveAttributeValue(Object dataObject, String fieldNames) throws Exception
	{
		...
		if (null == dataObject)
			throw new TreegixException("data object is null");
		...
		if (fieldNames.equals(""))
		{
			...
			{
				throw new TreegixException("Data object type cannot be converted to string.");
			}
		}
		...
		if (dataObject instanceof CompositeData)
		{
			...
			throw new TreegixException("unsupported data object type along the path: %s", dataObject.getClass());
	}
	...
	private void discoverAttributes(JSONArray counters, ObjectName filter, boolean propertiesAsMacros) throws Exception
	{
		for (ObjectName name : mbsc.queryNames(filter, null))
		{
			for (MBeanAttributeInfo attrInfo : mbsc.getMBeanInfo(name).getAttributes())
			{
				...
				{
					Object[] logInfo = {name, attrInfo.getName(), TreegixException.getRootCauseMessage(e)};
	...
	private void discoverBeans(JSONArray counters, ObjectName filter, boolean propertiesAsMacros) throws Exception
	{
		for (ObjectName name : mbsc.queryNames(filter, null))
		{
			...
			{
				logger.warn("bean processing '{}' failed: {}", name, TreegixException.getRootCauseMessage(e));
				logger.debug("error caused by", e);
			}
		}
	}
