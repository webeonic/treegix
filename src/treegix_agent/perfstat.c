		...
		zbx_perf_counter_lang_t lang, char **error)
{
	...
	treegix_log(LOG_LEVEL_DEBUG, "In %s() counter:'%s' interval:%d", __func__, counterpath, interval);
	...
	if (FAIL == added)
	{
		treegix_log(LOG_LEVEL_DEBUG, "%s() counter '%s' already exists", __func__, counterpath);
	}
...
out:
	...
	treegix_log(LOG_LEVEL_DEBUG, "End of %s(): %s", __func__, NULL == cptr ? "FAIL" : "SUCCEED");
...
void	collect_perfstat(void)
{
	...
	if (ERROR_SUCCESS != (pdh_status = PdhCollectQueryData(ppsd.pdh_query)))
	{
		...
		treegix_log(LOG_LEVEL_DEBUG, "%s() call to PdhCollectQueryData() failed: %s",
	...
	for (cptr = ppsd.pPerfCounterList; NULL != cptr; cptr = cptr->next)
	{
		...
		{
			treegix_log(LOG_LEVEL_DEBUG, "PDH_CALC_NEGATIVE_DENOMINATOR error occurred in counterpath '%s'."
		...
		{
			treegix_log(LOG_LEVEL_DEBUG, "PDH_CALC_NEGATIVE_VALUE error occurred in counterpath '%s'."
					" Value ignored", cptr->counterpath);
			continue;
		}
		...
		{
			treegix_log(LOG_LEVEL_WARNING, "cannot calculate performance counter value \"%s\": %s",
...
int	get_perf_counter_value(zbx_perf_counter_data_t *counter, int interval, double *value, char **error)
{
	...
	treegix_log(LOG_LEVEL_DEBUG, "In %s() path:%s interval:%d", __func__, counter->counterpath, interval);
