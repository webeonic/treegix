		...
		zbx_uint64_t *FirstID, zbx_uint64_t *LastID, char **error)
{
	...
	*LastID = 0;
	...
	{
		/* no data in eventlog */
		treegix_log(LOG_LEVEL_DEBUG, "first EvtNext failed:%s", strerror_from_system(GetLastError()));
		*FirstID = 1;
		*LastID = 1;
		numIDs = 0;
		*lastlogsize = 0;
...
static char	*expand_message6(const wchar_t *pname, EVT_HANDLE event)
{
	...
	if (TRUE != EvtFormatMessage(provider, event, 0, 0, NULL, EvtFormatMessageEvent, 0, NULL, &require))
	{
		if (ERROR_INSUFFICIENT_BUFFER == GetLastError())
		{
			...
			{
				treegix_log(LOG_LEVEL_DEBUG, "%s() cannot format message: %s", __func__,
 ...
 *                                                                            *
 * Parameters: server           - [IN] IP or Hostname of Treegix server        *
 *             port             - [IN] port of Treegix server                  *
...
#	define EVT_ARRAY_SIZE	100
	...
	if (NULL == *query)
	{
		treegix_log(LOG_LEVEL_DEBUG, "%s() no EvtQuery handle", __func__);
		goto out;
	}
