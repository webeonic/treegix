...
int	CONFIG_LISTEN_PORT		= TRX_DEFAULT_AGENT_PORT;
...
/* application TITLE */
const char	title_message[] = "treegix_agentd"
#if defined(_WIN64)
				" Win64"
#elif defined(_WIN32)
				" Win32"
...
const char	syslog_app_name[] = "treegix_agentd";
...
const char	*help_message[] = {
	"A Treegix daemon for monitoring of various server parameters.",
	"",
	"Options:",
	"  -c --config config-file        Path to the configuration file",
	"                                 (default: \"" DEFAULT_CONFIG_FILE "\")",
	"  -f --foreground                Run Treegix agent in foreground",
	...
	"Functions:",
	"",
	"  -i --install                   Install Treegix agent as service",
	"  -d --uninstall                 Uninstall Treegix agent from service",

	"  -s --start                     Start Treegix agent service",
	"  -x --stop                      Stop Treegix agent service",
#else
...
#endif
#ifdef _WINDOWS
	"Example: treegix_agentd -c C:\\treegix\\treegix_agentd.conf",
#else
	"Example: treegix_agentd -c /etc/treegix/treegix_agentd.conf",
#endif
	NULL	/* end of text */
};
...
unsigned char	program_type = TRX_PROGRAM_TYPE_AGENTD;

TRX_THREAD_LOCAL unsigned char	process_type	= 255;	/* TRX_PROCESS_TYPE_UNKNOWN */
TRX_THREAD_LOCAL int		process_num;
TRX_THREAD_LOCAL int		server_num	= 0;

static TRX_THREAD_ACTIVECHK_ARGS	*CONFIG_ACTIVE_ARGS = NULL;
...
int	get_process_info_by_thread(int local_server_num, unsigned char *local_process_type, int *local_process_num)
{
	...
	{
		*local_process_type = TRX_PROCESS_TYPE_COLLECTOR;
	...
	{
		*local_process_type = TRX_PROCESS_TYPE_LISTENER;
	...
	{
		*local_process_type = TRX_PROCESS_TYPE_ACTIVE_CHECKS;
...
static int	parse_commandline(int argc, char **argv, TRX_TASK_EX *t)
{
	int		i, ret = SUCCEED;
	char		ch;
#ifdef _WINDOWS
...
#endif
			case 'h':
				t->task = TRX_TASK_SHOW_HELP;
				goto out;
			case 'V':
				t->task = TRX_TASK_SHOW_VERSION;
				goto out;
			case 'p':
				if (TRX_TASK_START == t->task)
					t->task = TRX_TASK_PRINT_SUPPORTED;
				break;
			case 't':
				if (TRX_TASK_START == t->task)
				{
...
#ifdef _WINDOWS
			case 'i':
				t->task = TRX_TASK_INSTALL_SERVICE;
				break;
			case 'd':
				t->task = TRX_TASK_UNINSTALL_SERVICE;
				break;
			case 's':
				t->task = TRX_TASK_START_SERVICE;
				break;
			case 'x':
				t->task = TRX_TASK_STOP_SERVICE;
				break;
			case 'm':
				t->flags |= TRX_TASK_FLAG_MULTIPLE_AGENTS;
				break;
#endif
			default:
				t->task = TRX_TASK_SHOW_USAGE;
				goto out;
		}
	}
	...
	switch (t->task)
	{
		...
		case TRX_TASK_UNINSTALL_SERVICE:
		case TRX_TASK_START_SERVICE:
...
static void	set_defaults(void)
{
	...
	{
		treegix_log(LOG_LEVEL_WARNING, "both HostMetadata and HostMetadataItem defined, using [%s]",
				CONFIG_HOST_METADATA);
	}

	if (NULL != CONFIG_HOST_INTERFACE && NULL != CONFIG_HOST_INTERFACE_ITEM)
	{
		treegix_log(LOG_LEVEL_WARNING, "both HostInterface and HostInterfaceItem defined, using [%s]",
...
static void	trx_validate_config(TRX_TASK_EX *task)
{
	...
	{
		treegix_log(LOG_LEVEL_CRIT, "either active or passive checks must be enabled");
		err = 1;
	}
...
static int	trx_exec_service_task(const char *name, const TRX_TASK_EX *t)
{
	...
	{
		case TRX_TASK_INSTALL_SERVICE:
			ret = TreegixCreateService(name, t->flags & TRX_TASK_FLAG_MULTIPLE_AGENTS);
			break;
		case TRX_TASK_UNINSTALL_SERVICE:
			ret = TreegixRemoveService();
			break;
		case TRX_TASK_START_SERVICE:
			ret = TreegixStartService();
			break;
		case TRX_TASK_STOP_SERVICE:
			ret = TreegixStopService();
			break;
		default:
...
#	define TLS_FEATURE_STATUS	" NO"
	...
	if (TRX_IS_RUNNING())
	{
		/* Treegix agent service should either be stopped by the user in ServiceCtrlHandler() or */
		/* crash. If some thread has terminated normally, it means something is terribly wrong. */

		treegix_log(LOG_LEVEL_CRIT, "One thread has terminated unexpectedly (code:%lu). Exiting ...", res);
		THIS_SHOULD_NEVER_HAPPEN;
