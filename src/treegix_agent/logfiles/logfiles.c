...
static int	split_string(const char *str, const char *del, char **part1, char **part2)
{
	...
	treegix_log(LOG_LEVEL_DEBUG, "In %s() str:'%s' del:'%s'", __func__, str, del);
	...
	{
		treegix_log(LOG_LEVEL_DEBUG, "%s() cannot proceed: delimiter is out of range", __func__);
		goto out;
	}
...
static int	split_filename(const char *filename, char **directory, char **filename_regexp, char **err_msg)
{
	...
	for (sz = strlen(filename) - 1, separator = &filename[sz]; separator >= filename; separator--)
	{
		...
		treegix_log(LOG_LEVEL_DEBUG, "%s() %s", __func__, filename);
		treegix_log(LOG_LEVEL_DEBUG, "%s() %*s", __func__, separator - filename + 1, "^");
...
static void	resolve_old2new(char *old2new, int num_old, int num_new)
{
	...
	treegix_log(LOG_LEVEL_DEBUG, "resolve_old2new(): non-unique mapping");
		...
		struct st_logfile *new_files, int num_new, int use_ino, char **err_msg)
{
	...
	for (i = 0; i < num_old; i++)
	{
		for (j = 0; j < num_new; j++)
		{
			...
			treegix_log(LOG_LEVEL_DEBUG, "%s(): is_same_file(%s, %s) = %c", __func__,
	...
	for (; i < *logfiles_num; i++)
	{
		...
		if (st->st_mtime == (*logfiles)[i].mtime)
		{
			...
			if (0 == cmp)
			{
				...
				treegix_log(LOG_LEVEL_WARNING, "%s() file '%s' already added", __func__, filename);
				goto out;
			}
		...
		if (NULL == (p_nl = buf_find_newline(p, &p_next, p_end, cr, lf, szbyte)))
		{
			...
			else
			{
				...
				if (0 == *big_rec)
				{
					...
					treegix_log(LOG_LEVEL_WARNING, "Logfile contains a large record: \"%.64s\""
							" (showing only the first 64 characters). Only the first 256 kB"
							" will be analyzed, the rest will be ignored while Treegix agent"
							" is running.", value);
...
static void	adjust_mtime_to_clock(int *mtime)
{
	...
	if (*mtime > (now = time(NULL)))
	{
		...
		*mtime = (int)now;
		...
		treegix_log(LOG_LEVEL_WARNING, "System clock has been set back in time. Setting agent mtime %d "
		...
		int logfiles_num, int use_ino, int *start_idx)
{
	...
	for (i = 0; i < logfiles_num - 1; i++)
	{
		...
		{
			treegix_log(LOG_LEVEL_DEBUG, "ensure_order_if_mtimes_equal() swapping files '%s' and '%s'",
		...
		char **error)
{
	...
	/* do not flood Treegix server if file grows too fast */
