...
TRX_VECTOR_DECL(wmi_prop, trx_wmi_prop_t)
TRX_VECTOR_IMPL(wmi_prop, trx_wmi_prop_t)

TRX_PTR_VECTOR_DECL(wmi_instance, trx_vector_wmi_prop_t *)
TRX_PTR_VECTOR_IMPL(wmi_instance, trx_vector_wmi_prop_t *)

extern "C" static void	wmi_prop_clear(trx_wmi_prop_t *prop)
{
...
extern "C" static void	wmi_instance_clear(trx_vector_wmi_prop_t *wmi_inst_value)
{
	int	i;
	...
	trx_vector_wmi_prop_destroy(wmi_inst_value);
	trx_free(wmi_inst_value);
}

typedef int	(*trx_parse_wmi_t)(IEnumWbemClassObject *pEnumerator, trx_vector_wmi_instance_t *wmi_values,
		char **error);

extern "C" int	put_variant_json(const char *prop_json, const char *prop_err, VARIANT *vtProp, struct trx_json *jdoc,
		char **error);

static TRX_THREAD_LOCAL int	com_initialized = 0;

extern "C" int	trx_co_initialize()
{
...
extern "C" void	trx_co_uninitialize()
{
 ...
 *                                                                            *
 ******************************************************************************/
extern "C" static int	parse_first_first(IEnumWbemClassObject *pEnumerator, trx_vector_wmi_instance_t *wmi_values,
		char **error)
{
	...
	ULONG			uReturn = 0;
	HRESULT			hres;
	trx_vector_wmi_prop_t	*inst_val;
	trx_wmi_prop_t		prop;
	...
	if (FAILED(hres))
	{
		*error = trx_strdup(*error, "Cannot start WMI query result enumeration.");
		goto out;
	}

	vtProp = (VARIANT*) trx_malloc(NULL, sizeof(VARIANT));
	VariantInit(vtProp);
	hres = pclsObj->Next(0, NULL, vtProp, 0, 0);

	if (FAILED(hres))
	{
		*error = trx_strdup(*error, "Cannot parse WMI result field.");
		trx_free(vtProp);
		goto out;
	}
	...
	prop.name = NULL;
	prop.value = vtProp;
	inst_val = (trx_vector_wmi_prop_t*) trx_malloc(NULL, sizeof(trx_vector_wmi_prop_t));
	trx_vector_wmi_prop_create(inst_val);
	trx_vector_wmi_prop_append(inst_val, prop);
	trx_vector_wmi_instance_append(wmi_values, inst_val);
out:
	if (0 != pclsObj)
		pclsObj->Release();
 ...
 *                                                                            *
 ******************************************************************************/
extern "C" static int	parse_all(IEnumWbemClassObject *pEnumerator, trx_vector_wmi_instance_t *wmi_values,
		char **error)
{
	int	ret = SYSINFO_RET_FAIL;
	VARIANT	*vtProp = NULL;
	...
	while (pEnumerator && SUCCEEDED(hres))
	{
		...
		ULONG			uReturn = 0;
		HRESULT			hres;
		trx_vector_wmi_prop_t	*inst_val = NULL;
 ...
 *                                                                            *
 ******************************************************************************/
extern "C" int	trx_wmi_get_variant(const char *wmi_namespace, const char *wmi_query, trx_parse_wmi_t parse_value_cb,
		trx_vector_wmi_instance_t *wmi_values, char **error)
{
	IWbemLocator		*pLoc = 0;
	...
	if (FAILED(hres))
	{
		*error = trx_strdup(*error, "Cannot obtain WMI locator service.");
		goto exit;
	}

	wmi_namespace_wide = trx_utf8_to_unicode(wmi_namespace);
	hres = pLoc->ConnectServer(_bstr_t(wmi_namespace_wide), NULL, NULL, 0, NULL, 0, 0, &pService);
	trx_free(wmi_namespace_wide);

	if (FAILED(hres))
	{
		*error = trx_dsprintf(*error, "Cannot obtain %s WMI service.", wmi_namespace);
		goto exit;
	}
	...
	if (FAILED(hres))
	{
		*error = trx_strdup(*error, "Cannot set IWbemServices proxy.");
		goto exit;
	}

	wmi_query_wide = trx_utf8_to_unicode(wmi_query);
	hres = pService->ExecQuery(_bstr_t("WQL"), _bstr_t(wmi_query_wide),
			WBEM_FLAG_FORWARD_ONLY | WBEM_FLAG_RETURN_IMMEDIATELY, NULL, &pEnumerator);
	trx_free(wmi_query_wide);

	if (FAILED(hres))
	{
		*error = trx_dsprintf(*error, "Failed to execute WMI query %s.", wmi_query);
		goto exit;
	}
	...
	if (SYSINFO_RET_FAIL == ret && NULL == *error)
		*error = trx_strdup(*error, "Empty WMI search result.");
exit:
	if (0 != pEnumerator)
		pEnumerator->Release();
...
/******************************************************************************
 *                                                                            *
 * Function: trx_wmi_get                                                      *
 *                                                                            *
 * Purpose: wrapper function for trx_wmi_get_variant(), stores the retrieved  *
 ...
 *                                                                            *
 ******************************************************************************/
extern "C" void	trx_wmi_get(const char *wmi_namespace, const char *wmi_query, char **utf8_value)
{
	VARIANT				*vtProp;
	HRESULT				hres;
	trx_vector_wmi_instance_t	wmi_values;
	char				*error = NULL;

	trx_vector_wmi_instance_create(&wmi_values);

	if (SUCCEED != trx_co_initialize())
	{
		treegix_log(LOG_LEVEL_DEBUG, "cannot initialize COM library for querying WMI");
		goto out;
	}

	if (SYSINFO_RET_FAIL == trx_wmi_get_variant(wmi_namespace, wmi_query, parse_first_first, &wmi_values, &error))
	{
	...
	*utf8_value = trx_unicode_to_utf8((wchar_t *)_bstr_t(vtProp->bstrVal));
out:
	trx_vector_wmi_instance_clear_ext(&wmi_values, wmi_instance_clear);
	trx_vector_wmi_instance_destroy(&wmi_values);
	trx_free(error);
}
...
extern "C" int	WMI_GET(AGENT_REQUEST *request, AGENT_RESULT *result)
{
	...
	if (SUCCEED != trx_co_initialize())
	{
		SET_MSG_RESULT(result, trx_strdup(NULL, "Cannot initialize COM library."));
		return SYSINFO_RET_FAIL;
	}
	...
	if (V_ISARRAY(vtProp))
	{
		error = trx_strdup(error, "Cannot convert WMI array result.");
		goto out;
	}
	...
	switch (V_VT(vtProp))
	{
		...
		case VT_UINT:
			...
			if (FAILED(hres))
			{
				error = trx_dsprintf(error, "Cannot convert WMI result of type %d to VT_I8",
						V_VT(vtProp));
				goto out;
			}
		...
		case VT_R8:
			...
			if (FAILED(hres))
			{
				error = trx_dsprintf(error, "Cannot convert WMI result of type %d to VT_R8",
						V_VT(vtProp));
				goto out;
			}
		...
		default:
			...
			if (FAILED(hres))
			{
				error = trx_dsprintf(error, "Cannot convert WMI result of type %d to VT_BSTR",
						V_VT(vtProp));
				goto out;
			}

			SET_TEXT_RESULT(result, trx_unicode_to_utf8((wchar_t *)_bstr_t(V_BSTR(vtProp))));
			ret = SYSINFO_RET_OK;
 ...
 *                                                                            *
 ******************************************************************************/
extern "C" int	proc_arr_element(SAFEARRAY *sa, LONG *index, const char *prop_err, struct trx_json *jdoc,
		char **error)
{
	HRESULT	hres;
	BYTE	*pbData;
	...
	pbData = (BYTE*)trx_malloc(NULL, (size_t)SafeArrayGetElemsize(sa));
	hres = SafeArrayGetElement(sa, index, pbData);

	if (FAILED(hres))
	{
		*error = trx_dsprintf(*error, "Cannot get element from WMI array '%s'", prop_err);
		trx_free(pbData);
		return SYSINFO_RET_FAIL;
	}
	...
	if (FAILED(hres))
	{
		*error = trx_dsprintf(*error, "Cannot get element type from WMI array '%s'", prop_err);
		trx_free(pbData);
		return SYSINFO_RET_FAIL;
	}
	...
	switch(pvt &(~VT_ARRAY))
	{
		...
		case VT_CY:
			...
			if (FAILED(hres))
			{
				*error = trx_dsprintf(*error, "Cannot convert WMI property '%s' of "
		...
		case VT_VARIANT:
			...
			if (FAILED(hres))
			{
				*error = trx_dsprintf(*error, "Cannot copy array element of WMI property '%s'",
						prop_err);
				ret = SYSINFO_RET_FAIL;
			}
			...
			break;
		default:
			*error = trx_dsprintf(*error, "Unsupported type %d for array element of WMI property '%s'",
					pvt, prop_err);
			ret = SYSINFO_RET_FAIL;
			break;
	}
...
extern "C" int	convert_wmiarray_json(VARIANT *vtProp, const char *prop_name, ULONG dim, ULONG offset_dim,
		LONG **index, struct trx_json *jdoc, char **error)
{
	HRESULT		hres;
	...
	if (0 == dim)
	{
		...
		*index = (LONG*) trx_malloc(*index, (size_t)(sizeof(LONG) * dim));
		offset_dim = 1;
		trx_json_addarray(jdoc, prop_name);
	}
	else
		trx_json_addarray(jdoc, NULL);

	hres = SafeArrayGetLBound(sa, offset_dim, &lBound);

	if (FAILED(hres))
	{
		*error = trx_dsprintf(*error, "Cannot get index of first element from WMI array '%s'", prop_name);
		return SYSINFO_RET_FAIL;
	}
	...
	if (FAILED(hres))
	{
		*error = trx_dsprintf(*error, "Cannot get index of last element from WMI array '%s'", prop_name);
		return SYSINFO_RET_FAIL;
	}
		...
		char **error)
{
	...
	switch (V_VT(vtProp))
	{
		...
		case VT_UINT:
			...
			if (FAILED(hres))
			{
				*error = trx_dsprintf(*error, "Cannot convert WMI property '%s' of "
				...
				trx_json_adduint64(jdoc, prop_json, vtProp->llVal);
		...
		case VT_R8:
			...
			if (FAILED(hres))
			{
				*error = trx_dsprintf(*error, "Cannot convert WMI property '%s' of "
				...
				trx_json_addfloat(jdoc, prop_json, (double)vtProp->llVal);
		...
		default:
			...
			{
				str = trx_unicode_to_utf8((wchar_t *)_bstr_t(V_BSTR(vtProp)));
				trx_json_escape(&str);
				trx_json_addstring(jdoc, prop_json, str, TRX_JSON_TYPE_STRING);
				trx_free(str);
			}
 ...
 *                                                                            *
 ******************************************************************************/
extern "C" int	convert_wmi_json(trx_vector_wmi_instance_t *wmi_values, char **json_data, char **error)
{
	struct trx_json	j;
	int		inst_i, prop_i, ret = SYSINFO_RET_OK;

	trx_json_initarray(&j, wmi_values->values_num * wmi_values->values[0]->values_num * 50);
	...
	for (inst_i = 0; inst_i < wmi_values->values_num && SYSINFO_RET_OK == ret; inst_i++)
	{
		...
		for (prop_i = 0; prop_i < wmi_values->values[inst_i]->values_num && SYSINFO_RET_OK == ret; prop_i++)
		{
			...
			char	*prop_name = trx_unicode_to_utf8(
					(wchar_t *)_bstr_t(wmi_values->values[inst_i]->values[prop_i].name));

			trx_json_escape(&prop_name);
	...
	if (SYSINFO_RET_OK == ret)
	{
		...
		trx_str_memcpy_alloc(json_data, &len, &offset, j.buffer, j.buffer_size);
	}
...
extern "C" int	WMI_GETALL(AGENT_REQUEST *request, AGENT_RESULT *result)
{
	...
	if (SYSINFO_RET_OK == trx_wmi_get_variant(wmi_namespace, wmi_query, parse_all, &wmi_values, &error))
