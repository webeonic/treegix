...
{
	const TRX_FIELD	field = {"content_type", "1", NULL, NULL, 0, TRX_TYPE_INT, TRX_NOTNULL, 0};
...
static int	DBpatch_4010002(void)
{
	...
	if (TRX_DB_OK > DBexecute("update media_type set content_type=0"))
		return FAIL;
...
{
	const TRX_FIELD	field = {"error_handler", "0", NULL, NULL, 0, TRX_TYPE_INT, TRX_NOTNULL, 0};
...
{
	const TRX_FIELD	field = {"error_handler_params", "", NULL, NULL, 255, TRX_TYPE_CHAR, TRX_NOTNULL, 0};
...
{
	const TRX_FIELD	field = {"db_extension", "", NULL, NULL, 32, TRX_TYPE_CHAR, TRX_NOTNULL, 0};
...
{
	const TRX_FIELD	field = {"params", "", NULL, NULL, 0, TRX_TYPE_SHORTTEXT, TRX_NOTNULL, 0};
...
static int	DBpatch_4010013(void)
{
	...
	if (TRX_DB_OK > DBexecute("update profiles set idx='web.items.filter_groupids'"
...
static int	DBpatch_4010014(void)
{
	...
	if (TRX_DB_OK > DBexecute("update profiles set idx='web.items.filter_hostids'"
...
static int	DBpatch_4010015(void)
{
	...
	if (TRX_DB_OK > DBexecute("update profiles set idx='web.items.filter_inherited'"
...
static int	DBpatch_4010016(void)
{
	...
	if (TRX_DB_OK > DBexecute("delete from profiles where idx='web.triggers.filter_priority' and value_int='-1'"))
		return FAIL;
...
{
	const TRX_FIELD	field = {"host_source", "1", NULL, NULL, 0, TRX_TYPE_INT, TRX_NOTNULL, 0};
...
{
	const TRX_FIELD	field = {"name_source", "0", NULL, NULL, 0, TRX_TYPE_INT, TRX_NOTNULL, 0};
...
{
	const TRX_FIELD	field = {"druleid", NULL, "drules", "druleid", 0, 0, 0, TRX_FK_CASCADE_DELETE};
...
{
	const TRX_FIELD	field = {"height", "2", NULL, NULL, 0, TRX_TYPE_INT, TRX_NOTNULL, 0};
...
static int	DBpatch_4010025(void)
{
	...
	if (TRX_DB_OK > DBexecute("delete from ids where table_name='proxy_history'"))
		return FAIL;

	result = DBselect("select max(id) from proxy_history");

	if (NULL != (row = DBfetch(result)))
		TRX_DBROW2UINT64(nextid, row[0]);
	else
		nextid = 0;

	DBfree_result(result);

	if (0 != nextid && TRX_DB_OK > DBexecute("insert into ids values ('proxy_history','history_lastid'," TRX_FS_UI64
			")", nextid))
	{
		return FAIL;
	}
...
static int	DBpatch_4010026(void)
{
	...
	if (TRX_DB_OK > DBexecute("update hosts set status=1"))
		return FAIL;
...
{
	const TRX_FIELD	field = {"details", "", NULL, NULL, 255, TRX_TYPE_CHAR, TRX_NOTNULL, 0};
