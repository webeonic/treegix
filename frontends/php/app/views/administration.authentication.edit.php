...
$auth_tab = (new CFormList('list_auth'))
	->addRow(new CLabel(_('Default authentication'), 'authentication_type'),
		(new CRadioButtonList('authentication_type', (int) $data['authentication_type']))
			...
			->addValue(_x('Internal', 'authentication'), TRX_AUTH_INTERNAL)
			->addValue(_('LDAP'), TRX_AUTH_LDAP)
			->setModern(true)
			->removeId()
	);
...
$http_tab = (new CFormList('list_http'))
	->addRow(new CLabel(_('Enable HTTP authentication'), 'http_auth_enabled'),
		(new CCheckBox('http_auth_enabled', TRX_AUTH_HTTP_ENABLED))
			->setChecked($data['http_auth_enabled'] == TRX_AUTH_HTTP_ENABLED)
			->setUncheckedValue(TRX_AUTH_HTTP_DISABLED)
	)
	->addRow(new CLabel(_('Default login form'), 'http_login_form'),
		(new CComboBox('http_login_form', $data['http_login_form'], null, [
			TRX_AUTH_FORM_TREEGIX => _('Treegix login form'),
			TRX_AUTH_FORM_HTTP => _('HTTP login form')
		]))->setEnabled($data['http_auth_enabled'] == TRX_AUTH_HTTP_ENABLED)
	)
	...
	->addRow(new CLabel(_('Case sensitive login'), 'http_case_sensitive'),
		(new CCheckBox('http_case_sensitive', TRX_AUTH_CASE_SENSITIVE))
			->setChecked($data['http_case_sensitive'] == TRX_AUTH_CASE_SENSITIVE)
			->setEnabled($data['http_auth_enabled'] == TRX_AUTH_HTTP_ENABLED)
			->setUncheckedValue(TRX_AUTH_CASE_INSENSITIVE)
);
...
$ldap_tab = (new CFormList('list_ldap'))
	->addRow(new CLabel(_('Enable LDAP authentication'), 'ldap_configured'),
		$data['ldap_error']
		? (new CLabel($data['ldap_error']))->addClass(TRX_STYLE_RED)
		: (new CCheckBox('ldap_configured', TRX_AUTH_LDAP_ENABLED))
			->setChecked($data['ldap_configured'] == TRX_AUTH_LDAP_ENABLED)
			->setUncheckedValue(TRX_AUTH_LDAP_DISABLED)
	)
	...
	->addRow(new CLabel(_('Case sensitive login'), 'ldap_case_sensitive'),
		(new CCheckBox('ldap_case_sensitive', TRX_AUTH_CASE_SENSITIVE))
			->setChecked($data['ldap_case_sensitive'] == TRX_AUTH_CASE_SENSITIVE)
			->setEnabled($data['ldap_configured'] == TRX_AUTH_LDAP_ENABLED)
			->setUncheckedValue(TRX_AUTH_CASE_INSENSITIVE)
	)
